...
namespace LGTVDeviceListener {
	...
	std::wstring ToWideString(std::string_view input, UINT codePage) {
		...
		const auto size = ::MultiByteToWideChar(codePage, MB_ERR_INVALID_CHARS, input.data(), int(input.size()), NULL, 0);
		...
		if (::MultiByteToWideChar(codePage, MB_ERR_INVALID_CHARS, input.data(), int(input.size()), result.data(), int(result.size())) != int(result.size()))
